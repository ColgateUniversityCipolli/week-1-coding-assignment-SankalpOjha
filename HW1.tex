...
\begin{alltt}
\hlkwd{canvas_collatz}\hldef{(}
  \hlkwc{colors} \hldef{=} \hlkwd{colorPalette}\hldef{(}\hlsng{"retro4"}\hldef{),}
  \hlkwc{background} \hldef{=} \hlsng{"#dbfada"}\hldef{,}
  \hlkwc{n} \hldef{=} \hlnum{37}\hldef{,}
  \hlkwc{angle.even} \hldef{=} \hlnum{0.74}\hldef{,}
  \hlkwc{angle.odd} \hldef{=} \hlopt{-}\hlnum{0.075}
\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Code run in Question 2 Part A}
\hlcom{#x <- seq(from = 0, to = 10, by = 1)}

\hldef{eq} \hlkwb{<-} \hlnum{2}\hlopt{^}\hldef{\{x}\hlopt{+}\hlnum{1}\hldef{\}}\hlopt{+}\hlnum{2}\hlopt{^}\hldef{\{x}\hlopt{-}\hlnum{1}\hldef{\}}
\end{alltt}
\end{kframe}
\end{knitrout}
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\hlcom{#Code run in Question 2 Part B}
\hlcom{#eq <- 2^\{x+1\}+2^\{x-1\}}

\hlkwd{which}\hldef{(eq}\hlopt{==}\hlnum{40}\hldef{)}
\end{alltt}
\begin{verbatim}
## [1] 5
\end{verbatim}
...
\textbf{Solution:} The 5th element in the vector is the solution which means x must equal 4 (x = 4) for the equation to equal 40.
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % QUESTION 2e
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \item Explain why this approach wouldn't work for something like $3^{x+2} + 5 (3^x) = 84$ where the solution is $x \approx 1.6309$.\\
\textbf{Solution:} As 1.6309 is \underline{\textbf{NOT}} an integer! The method used earlier will fail for this question as 1.6309 is \underline{\textbf{NOT}} a value in the vector.
\end{enumerate}
\end{enumerate}
